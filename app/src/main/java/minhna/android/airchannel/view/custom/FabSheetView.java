...
import minhna.android.airchannel.view.custom.animator.AlphaAnimator;
import minhna.android.airchannel.view.custom.animator.ArcAnimator;
...
public class FabSheetView {
    ...
    private FabSheetView(FloatingActionButton fab, View sheet, int backgroundColor, int degree, long duration, Integer fabMaxBottom,
        ...
        onAnimationEnd(mFabAnimation, new OnAnimationEndListener() {
            ...
            public void onAnimationEnd(Animator animation) {
                ...
                if (mOnDismissListener != null) {
                    mOnDismissListener.onDismiss(FabSheetView.this);
                }
            }
        });
        ...
        onAnimationEnd(mSheetAnimation, new OnAnimationEndListener() {
            ...
            public void onAnimationEnd(Animator animation) {
                if (mOnShowListener != null) {
                    mOnShowListener.onShow(FabSheetView.this);
                }
            }
        });
    ...
    private Animator createSheetDismissAnimation() {
        ...
        Animator animator = createCircularReveal(vSheetContainer, dx, dyFab, radius, mFabInfo.radius);
        animator.setInterpolator(ACCELERATE);
        return animator;
    }
    ...
    private Animator createSheetShowAnimation() {
        ...
        int dyFab = dy + (int) (target.y - (mFabMaxBottom != null ? (float) Math.min(mFabMaxBottom, target.y) : target.y));
        float radius = (float) Math.hypot(dx, Math.max(dy, dyFab));
        Animator animator = createCircularReveal(vSheetContainer, dx, dyFab, mFabInfo.radius, radius);
        animator.setInterpolator(DECELERATE);
        return animator;
    }
    ...
    public interface OnDismissListener {
        ...
        void onDismiss(FabSheetView fsw);
    }

    public interface OnShowListener {

        void onShow(FabSheetView fsw);
    }
    ...
    public static class Builder {
        ...
        public FabSheetView build() {
            return new FabSheetView(vFab, vSheet, mBackgroundColor, mDegree, mDuration, mFabMaxBottom, mOnShowListener, mOnDismissListener);
        }
    ...
    private static class FabInfo {
        ...
        FabInfo(FloatingActionButton fab) {
            ...
            radius = Math.min(width / 2, DimenUtil.dptopx(null, FAB_SIZE));
        }
    ...
    @TargetApi(Build.VERSION_CODES.HONEYCOMB)
    public Animator createCircularReveal(CircularRevealFrameLayout view, int centerX, int centerY, float startRadius,
                                         float endRadius) {
